...
export async function GET(_req: NextRequest) {
  ...
  if (!AgencyID || !UserID || !Password) {
    return new Response(
      JSON.stringify({ error: 'Faltan credenciales en variables de entorno.' }),
      { status: 400 }
    );
  }
  ...
  try {
    const { data: xml } = await axios({
      ...
      headers: {
        ...
        'SOAPAction': 'http://webservices.aramix.es/IServiceAereo/AirlineCarriersList'
      },
      data: soapEnvelope
    });
